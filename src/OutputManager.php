...
class OutputManager 
{
  ...
  /**
   *
   * @var \wsdl2php\Config A reference to the config 
   */
  private $config;
  
  /**
   *
   * @var \phpSource\PhpFile The file object to use for saving the files
   */
  private $file;
   ...
   */
  public function __construct(\wsdl2php\Config $config)
  {
    $this->config = $config;
    $this->dir = '';
   ...
   * @param array $types
   */
  public function save(\phpSource\PhpClass $service, array $types)
  {
    $this->setOutputDirectory();
    
    if ($this->config->getOneFile())
    {
      $this->file = new \phpSource\PhpFile($service->getIdentifier());
      $this->addNamespace();
   ...
   *
   * @throws \wsdl2php\Exception If the dir can't be created and dont already exists
   */
  private function setOutputDirectory()
  {
    ...
    if (\is_dir($outputDirectory) == false && \is_file($outputDirectory) == false)
    {
      ...
      {
        throw new \wsdl2php\Exception('Could not create output directory and it does not exist!');
      }
    }
   ...
   */
  private function addClassToFile(\phpSource\PhpClass $class)
  {
    ...
    if ($this->isValidClass($class))
    {
      if ($this->file == null)
      {
        $this->file = new \phpSource\PhpFile($class->getIdentifier());
        $this->addNamespace();
      }
   ...
   * @param PhpClass $class
   */
  private function saveClassToFile(\phpSource\PhpClass $class)
  {
   ...
   */
  private function isValidClass(\phpSource\PhpClass $class)
  {
